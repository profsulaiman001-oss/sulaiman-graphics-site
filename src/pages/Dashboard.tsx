                ...
                <div className="p-5 flex flex-wrap justify-between items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className={`w-10 h-10 flex items-center justify-center rounded-full border transition relative ${
                    ...
                    {project.file_url ? (
                      <a 
                        ...
                        className="bg-primary hover:opacity-90 text-white font-medium text-xs px-4 py-2.5 rounded-xl transition flex items-center gap-1"
                      >
                        <Download size={14} /> Download Design
                      </a>
                    ) : !isAdmin && (
                      <span className="text-xs text-muted-foreground italic border border-border px-3 py-2.5 rounded-xl">
                        Design in progress...
                      </span>
                    )}
                      ...
                      <div className="flex items-center gap-2">
                        {['Pending', 'In Progress', 'Completed'].map((status) => (
                          <button
                            ...
                            className={`text-xs px-3 py-2.5 rounded-xl border transition font-medium ${
                    ...
                    {!isAdmin && project.file_url && !project.client_approval && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleClientApproval(project.id, 'Approved')}
                          className="bg-background border border-border hover:border-green-500 hover:text-green-500 text-muted-foreground font-medium text-xs px-3 py-2.5 rounded-xl transition"
                        >
                          Approve
                        </button>
                        <button
                          onClick={() => handleClientApproval(project.id, 'Revision Requested')}
                          className="bg-background border border-border hover:border-yellow-600 hover:text-yellow-600 text-muted-foreground font-medium text-xs px-3 py-2.5 rounded-xl transition"
                        >
                          Request Revision
                        </button>
                      </div>
                    ...
                    {!isAdmin && project.client_approval && (
                      <span className={`text-xs px-3 py-2.5 rounded-xl font-medium border ${
                        project.client_approval === 'Approved' 
                          ? 'bg-green-500/10 text-green-500 border-green-500/20' 
                          : 'bg-yellow-500/10 text-yellow-600 border-yellow-500/20'
                      }`}>
                        {project.client_approval === 'Approved' ? '✓ You Approved This' : '✍️ Revision Requested'}
                      </span>
                    )}
                  ...
                  <div className="flex items-center gap-2 border-l border-border pl-4 ml-2">
                    {editingId === project.id ? (
                      <>
                        ...
                        <button onClick={() => setEditingId(null)} className="w-9 h-9 flex items-center justify-center rounded-lg border border-border text-muted-foreground bg-background hover:bg-muted transition">
                          <XCircle size={16}/>
                        </button>
                      </>
                    ) : (
                      <>
                        {isAdmin && (
                          <label className="w-9 h-9 flex items-center justify-center rounded-lg border border-border text-muted-foreground bg-background hover:bg-muted transition cursor-pointer">
                            <input
                              type="file"
                              className="hidden"
                              onChange={(e) => {
                    ...
                    >
                      <div className="p-4">
                        <div className="flex justify-between items-center mb-3">
                          <h4 className="text-xs font-semibold uppercase text-muted-foreground">Project Discussion</h4>
                        </div>

                        <div className="bg-background border border-border rounded-xl p-3 max-h-40 overflow-y-auto mb-3 space-y-2">
                          {comments.length === 0 ? (
                            <p className="text-xs text-muted-foreground italic text-center py-2">No messages yet. Start the conversation!</p>
                          ) : (
