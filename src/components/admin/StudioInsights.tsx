...
export const StudioInsights = () => {
  ...
  return (
    <div className="navbar-safe-pt min-h-screen bg-[#020617] text-white p-4 md:p-12 pb-32 font-sans selection:bg-blue-500 uppercase">
      ...
      <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 mb-12">
        <div>
          ...
          <h1 className="text-6xl md:text-9xl font-black italic tracking-tighter leading-[0.75] text-white/90">
            Design <span className="text-blue-600">Studio</span>
          </h1>
        </div>
        
        <button onClick={() => setIsLogging(true)} className="group bg-blue-600 px-12 py-7 rounded-2xl transition-all hover:scale-105 shadow-2xl shadow-blue-600/20">
          <span className="relative flex items-center gap-4 text-[11px] font-black italic tracking-[0.3em] text-white">
      ...
      <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 auto-rows-[160px] gap-4 mb-10">
        ...
        <div className="lg:col-span-9 lg:row-span-1 modular-border-neon p-6 bg-[#070e1b] flex items-center gap-8">
           <div className="min-w-[140px]">
              ...
              <span className="text-4xl font-black italic">{pulseData[pulseData.length-1]?.rate.toFixed(0) || 0}%</span>
           </div>
           <div className="h-full flex-1">
             <ResponsiveContainer width="100%" height="100%">
               <LineChart data={pulseData}>
                 <Line type="monotone" dataKey="rate" stroke="#3b82f6" strokeWidth={4} dot={false} />
               </LineChart>
             </ResponsiveContainer>
           </div>
        </div>
          ...
          <div className="h-[85%]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sourceBarData} layout="vertical">
                ...
                <YAxis dataKey="name" type="category" tick={{fill: '#71717a', fontSize: 10, fontWeight: '900'}} width={80} />
                <Tooltip cursor={{fill: 'rgba(255,255,255,0.02)'}} contentStyle={{backgroundColor: '#0a0f1d', border: 'none'}}/>
                <Bar dataKey="sent" fill="#1e293b" radius={[0, 4, 4, 0]} barSize={14} />
                <Bar dataKey="replies" fill="#3b82f6" radius={[0, 4, 4, 0]} barSize={14} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <div key={i} className="group w-full flex justify-between items-center bg-[#0a0f1d]/40 border border-white/5 p-6 rounded-2xl hover:border-blue-500/30 transition-all">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <span className="text-[9px] font-black text-blue-500/60 tracking-[0.2em]">{proj.project_category}</span>
                  </div>
                </div>
